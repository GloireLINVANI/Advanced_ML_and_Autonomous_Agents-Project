...
def cem_uncorrelated(
    ...
    for iteration_index in range(0, max_iterations):
        ...
        x_array = np.random.normal(mean_array, np.sqrt(var_array), size=(sample_size, len(mean_array)))  # TODO...

        # EVALUATE SAMPLES AND EXTRACT THE BEST ONES ("ELITE") ################

        score_array = np.array([objective_function(x) for x in x_array])  # TODO...

        sorted_indices_array = np.argsort(
            score_array)  # TODO...             # Sort from the lower score to the higher one
        elite_indices_array = sorted_indices_array[
                              :n_elite]  # TODO...              # Recall: we *minimize* the objective function thus we take the samples that are at the begining of the sorted_indices

        elite_x_array = x_array[elite_indices_array]  # TODO...

        # FIT THE NORMAL DISTRIBUTION ON THE ELITE POPULATION #################

        mean_array = np.mean(elite_x_array, axis=0)  # TODO...
        var_array = np.var(elite_x_array, axis=0)  # TODO...
        score = np.min(score_array)  # TODO...
        ...
        if num_evals_for_stop is not None:
            score = objective_function(mean_array)  # TODO...
...
def saes_1_1(
    ...
    for iteration_index in range(0, max_iterations):
        ...
        new_sigma_array = sigma_array * np.exp(tau * np.random.normal(0, 1, size=d))  # TODO...

        # 2. Mutation of x (current solution)
        new_x_array = x_array + new_sigma_array * np.random.normal(0, 1, size=d)  # TODO...

        # 3. Eval f(x')
        new_score = objective_function(new_x_array)  # TODO...
        ...
        if new_score <= score:  # You may try `new_score < score` for less exploration
            score = new_score  # TODO...
            x_array = new_x_array.copy()  # TODO...
            sigma_array = new_sigma_array.copy()  # TODO...
        ...
        if num_evals_for_stop is not None:
            score = objective_function(x_array)  # TODO...
...
print("Average Reward (DQN v2 (1+1)-SA-ES): ",
      -objective_function.eval(reinforce_policy_nn_saes_1_1, num_episodes=NUM_EPISODES, max_time_steps=200))
